...
class VoiceService
{
    public const VOICE_FOLDER = 'voices';
    ...
    public function getOrCreateRootFolder(User $user, StorageConnection $connection): ?UserDocument
    {
        ...
        if ($connection->provider === 'google_drive') {
            ...
            $result = $this->driveService->createFolder($accessToken, self::VOICE_FOLDER, $parentExternalId);
            if (! $result) {
                return null;
            }
            ...
            $result = $this->wasabiService->createFolder($credentials, self::VOICE_FOLDER, $parentPrefix !== '' ? $parentPrefix : null);
            if (! $result) {
                return null;
            }
        ...
        return UserDocument::create([
            ...
            'name' => self::VOICE_FOLDER,
            'original_name' => self::VOICE_FOLDER,
            'path' => $externalId,
    ...
    public function uploadFile(UploadedFile $file, UserDocument $parentFolder, User $user, StorageConnection $connection): ?UserDocument
    {
        if ($connection->provider === 'google_drive') {
            ...
            if (! $result) {
                Log::warning('Voice: Drive upload failed', ['error' => $this->driveService->getLastError()]);
                return null;
            }
            $externalId = $result['id'];
            ...
            if (! $result) {
                Log::warning('Voice: Wasabi upload failed', ['error' => $this->wasabiService->getLastError()]);
                return null;
            }
            $externalId = $result['key'];
            $name = $result['name'];
