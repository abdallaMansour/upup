...
<div class="container-xxl flex-grow-1 container-p-y">
    @php $editStage = $achievement->childhoodStage ?? null; @endphp
    @include('dashboard.partials.breadcrumb', [
        'items' => $editStage
            ? [
                ...
                ['label' => __('documents.achievements'), 'url' => route('dashboard.achievements.index', ['stage' => $editStage->id])],
    ...
    <h4 class="mb-4">{{ __('documents.achievement_edit_title') }}</h4>
            ...
            <form action="{{ route('dashboard.achievements.update', $achievement) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="row g-3 mb-4">
                    ...
                    <div class="col-md-4">
                        ...
                        <select class="form-select @error('type') is-invalid @enderror" id="type" name="type" required>
                            @foreach (\App\Models\UserAchievement::TYPES as $value => $label)
                                <option value="{{ $value }}" @selected(old('type', $achievement->type) === $value)>{{ __('documents.achievement_types.' . $value) }}</option>
                            @endforeach
                        </select>
                        @error('type')
                            <div class="invalid-feedback">{{ $message }}</div>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="text" class="form-control @error('academic_year') is-invalid @enderror" id="academic_year" name="academic_year" value="{{ old('academic_year', $achievement->academic_year) }}" placeholder="{{ __('documents.achievement_academic_year_placeholder') }}">
                        @error('academic_year')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                        ...
                        @if ($achievement->certificateImageDocument)
                            <div class="mb-2">
                                <a href="{{ $achievement->certificateImageDocument->view_url }}" target="_blank" class="btn btn-sm btn-outline-primary">
                                    <i class="bx bx-image me-1"></i> {{ __('documents.achievement_view_current_certificate') }}
                                </a>
                            </div>
                        @endif
                    ...
                    <div class="col-md-6">
                        <label for="photos" class="form-label">{{ __('documents.achievement_add_new_photos') }}</label>
                        ...
                        <small class="text-muted">{{ __('documents.achievement_can_add_photos_without_delete') }}</small>
                        @error('photos.*')
                            <div class="invalid-feedback d-block">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="col-12">
                        <label for="videos" class="form-label">{{ __('documents.achievement_add_new_videos') }}</label>
                        ...
                        <small class="text-muted">{{ __('documents.achievement_can_add_videos_without_delete') }}</small>
                        @error('videos.*')
                            <div class="invalid-feedback d-block">{{ $message }}</div>
                        @enderror
                    </div>
