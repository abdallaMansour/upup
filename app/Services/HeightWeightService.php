...
class HeightWeightService
{
    public const HEIGHT_WEIGHT_FOLDER = 'height_weight';
    ...
    public function getOrCreateRootFolder(User $user, StorageConnection $connection): ?UserDocument
    {
        $existing = UserDocument::where('user_id', $user->id)
            ...
            ->where('type', 'folder')
            ->where('name', self::HEIGHT_WEIGHT_FOLDER)
            ->first();
        ...
        if ($connection->provider === 'google_drive') {
            ...
            $result = $this->driveService->createFolder($accessToken, self::HEIGHT_WEIGHT_FOLDER, $parentExternalId);
            if (! $result) {
                return null;
            }
            ...
            $result = $this->wasabiService->createFolder($credentials, self::HEIGHT_WEIGHT_FOLDER, $parentPrefix !== '' ? $parentPrefix : null);
            if (! $result) {
                return null;
            }
        ...
        return UserDocument::create([
            ...
            'name' => self::HEIGHT_WEIGHT_FOLDER,
            'original_name' => self::HEIGHT_WEIGHT_FOLDER,
            'path' => $externalId,
    ...
    public function uploadFile(UploadedFile $file, UserDocument $parentFolder, User $user, StorageConnection $connection): ?UserDocument
    {
        if ($connection->provider === 'google_drive') {
            ...
            if (! $result) {
                Log::warning('HeightWeight: Drive upload failed', ['error' => $this->driveService->getLastError()]);
                return null;
            }
            $externalId = $result['id'];
            ...
            if (! $result) {
                Log::warning('HeightWeight: Wasabi upload failed', ['error' => $this->wasabiService->getLastError()]);
                return null;
            }
            $externalId = $result['key'];
            $name = $result['name'];
