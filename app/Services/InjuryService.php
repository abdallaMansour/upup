...
class InjuryService
{
    public const INJURY_FOLDER = 'injuries';
    ...
    public function getOrCreateRootFolder(User $user, StorageConnection $connection): ?UserDocument
    {
        ...
        if ($connection->provider === 'google_drive') {
            ...
            $result = $this->driveService->createFolder($accessToken, self::INJURY_FOLDER, $parentExternalId);
            if (! $result) {
                return null;
            }
            ...
            $result = $this->wasabiService->createFolder($credentials, self::INJURY_FOLDER, $parentPrefix !== '' ? $parentPrefix : null);
            if (! $result) {
                return null;
            }
        ...
        return UserDocument::create([
            ...
            'name' => self::INJURY_FOLDER,
            'original_name' => self::INJURY_FOLDER,
            'path' => $externalId,
    ...
    public function uploadFile(UploadedFile $file, UserDocument $parentFolder, User $user, StorageConnection $connection): ?UserDocument
    {
        if ($connection->provider === 'google_drive') {
            ...
            if (! $result) {
                Log::warning('Injury: Drive upload failed', ['error' => $this->driveService->getLastError()]);
            ...
            if (! $result) {
                Log::warning('Injury: Wasabi upload failed', ['error' => $this->wasabiService->getLastError()]);
