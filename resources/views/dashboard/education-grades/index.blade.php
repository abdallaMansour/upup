        ...
        @if(auth('admin')->check() && auth('admin')->user()->hasPermission('education-stages.manage'))
            <div class="d-flex gap-2">
                <a href="{{ route('dashboard.education-stages.index') }}" class="btn btn-label-secondary">
                    <i class="bx bx-book-reader me-1"></i> المراحل التعليمية
                </a>
                <a href="{{ route('dashboard.education-grades.create') }}" class="btn btn-primary">
                    <i class="bx bx-plus me-1"></i> إضافة صف
                </a>
            </div>
        @endif
    </div>
                                ...
                                <td>
                                    <div class="dropdown">
                                        ...
                                        <div class="dropdown-menu dropdown-menu-end">
                                            <a class="dropdown-item" href="{{ route('dashboard.education-grades.edit', $grade) }}">
                                                <i class="bx bx-edit-alt me-2"></i> تعديل
                                            </a>
                                            <form action="{{ route('dashboard.education-grades.destroy', $grade) }}" method="POST" class="d-inline" onsubmit="return confirm('هل أنت متأكد؟');">
                                                @csrf
                                                @method('DELETE')
