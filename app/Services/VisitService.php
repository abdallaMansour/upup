...
class VisitService
{
    public const VISIT_FOLDER = 'visits';
    ...
    public function getOrCreateRootFolder(User $user, StorageConnection $connection): ?UserDocument
    {
        ...
        if ($connection->provider === 'google_drive') {
            ...
            $result = $this->driveService->createFolder($accessToken, self::VISIT_FOLDER, $parentExternalId);
            if (! $result) {
                return null;
            }
            ...
            $result = $this->wasabiService->createFolder($credentials, self::VISIT_FOLDER, $parentPrefix !== '' ? $parentPrefix : null);
            if (! $result) {
                return null;
            }
        ...
        return UserDocument::create([
            ...
            'name' => self::VISIT_FOLDER,
            'original_name' => self::VISIT_FOLDER,
            'path' => $externalId,
    ...
    public function uploadFile(UploadedFile $file, UserDocument $parentFolder, User $user, StorageConnection $connection): ?UserDocument
    {
        if ($connection->provider === 'google_drive') {
            ...
            if (! $result) {
                Log::warning('Visit: Drive upload failed', ['error' => $this->driveService->getLastError()]);
            ...
            if (! $result) {
                Log::warning('Visit: Wasabi upload failed', ['error' => $this->wasabiService->getLastError()]);
