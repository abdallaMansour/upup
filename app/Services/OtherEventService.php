...
class OtherEventService
{
    public const OTHER_EVENT_FOLDER = 'other_events';
    ...
    public function getOrCreateRootFolder(User $user, StorageConnection $connection): ?UserDocument
    {
        $existing = UserDocument::where('user_id', $user->id)
            ...
            ->where('type', 'folder')
            ->where('name', self::OTHER_EVENT_FOLDER)
            ->first();
        ...
        if ($connection->provider === 'google_drive') {
            ...
            $result = $this->driveService->createFolder($accessToken, self::OTHER_EVENT_FOLDER, $parentExternalId);
            if (! $result) {
                return null;
            }
            ...
            $result = $this->wasabiService->createFolder($credentials, self::OTHER_EVENT_FOLDER, $parentPrefix !== '' ? $parentPrefix : null);
            if (! $result) {
                return null;
            }
        ...
        return UserDocument::create([
            ...
            'name' => self::OTHER_EVENT_FOLDER,
            'original_name' => self::OTHER_EVENT_FOLDER,
            'path' => $externalId,
    ...
    public function uploadFile(UploadedFile $file, UserDocument $parentFolder, User $user, StorageConnection $connection): ?UserDocument
    {
        if ($connection->provider === 'google_drive') {
            ...
            if (! $result) {
                Log::warning('OtherEvent: Drive upload failed', ['error' => $this->driveService->getLastError()]);
            ...
            if (! $result) {
                Log::warning('OtherEvent: Wasabi upload failed', ['error' => $this->wasabiService->getLastError()]);
