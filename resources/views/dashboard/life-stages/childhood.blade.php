...
<div class="container-xxl flex-grow-1 container-p-y">
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form action="{{ $stage ? route('dashboard.life-stages.childhood.update') : route('dashboard.life-stages.childhood.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
                @if ($stage)
                    @method('PUT')
                @endif
                ...
                <button type="submit" class="btn btn-primary">
                    <i class="bx bx-save me-1"></i> {{ $stage ? 'تحديث' : 'حفظ' }}
                </button>
                @if (!($primaryConnection ?? null))
