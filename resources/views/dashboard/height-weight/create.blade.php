...
<div class="container-xxl flex-grow-1 container-p-y">
    @include('dashboard.partials.breadcrumb', [
        'items' => ($stage ?? null)
            ? [
                ...
                ['label' => __('dashboard.breadcrumb.height_weight'), 'url' => route('dashboard.height-weight.index', ['stage' => $stage->id])],
    ...
    <h4 class="mb-4">{{ __('life_stages.add_record') }}</h4>
            ...
            <form action="{{ route('dashboard.height-weight.store', ($stage ?? null) ? ['stage' => $stage->id] : []) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="row g-3 mb-4">
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="number" step="0.01" class="form-control @error('height') is-invalid @enderror" id="height" name="height" value="{{ old('height') }}" placeholder="{{ __('life_stages.example', ['value' => '170']) }}">
                        @error('height')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="col-md-6">
                        ...
                        <input type="number" step="0.01" class="form-control @error('weight') is-invalid @enderror" id="weight" name="weight" value="{{ old('weight') }}" placeholder="{{ __('life_stages.example', ['value' => '70']) }}">
                        @error('weight')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
