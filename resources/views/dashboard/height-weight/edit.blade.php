            ...
            <form action="{{ route('dashboard.height-weight.update', $heightWeight) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="row g-3 mb-4">
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="number" step="0.01" class="form-control @error('height') is-invalid @enderror" id="height" name="height" value="{{ old('height', $heightWeight->height) }}" placeholder="مثال: 170">
                        @error('height')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="col-md-6">
                        ...
                        <input type="number" step="0.01" class="form-control @error('weight') is-invalid @enderror" id="weight" name="weight" value="{{ old('weight', $heightWeight->weight) }}" placeholder="مثال: 70">
                        @error('weight')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <small class="text-muted">حد أقصى 50 ميجابايت (اترك فارغاً للاحتفاظ بالفيديو الحالي)</small>
                        @error('video')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
